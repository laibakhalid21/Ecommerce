...
function ProductCard({ product, setmsg }) {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      else stars.push(<span key={i} className="text-yellow-400">✩</span>);
    }
    return stars;
  };

  return (
    <div className="border bg-slate-100 rounded-lg shadow p-4 hover:scale-105 transition duration-200 ease-linear hover:shadow-xl flex flex-col justify-between">
      ...
      <div className="mt-1">{renderStars(product.rating.rate)}</div>
